namespace HMSCore.Areas.Admin.Validators.Messages
{
    public class ValidatorConstants
    {
        public const string validateCountryNameErrMsg = "Country already exist!";

        public const string validateCityNameErrMsg = "City already exist!";

        public const string validateCountryIdErrMsg = "Invalid country!";

        public const string minLength = "Minimum length should be {1}!";

        public const string maxLength = "Maximum length should be {1}!";

        public const string phoneErrMsg = "Not valid phone number!";

    }
}